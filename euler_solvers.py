...
# Toro eqns. 10.21
def hlle_flux(priml, primr, s=0.0):
    rhol, ul, pl = priml
    rhor, ur, pr = primr
...
def hlle_flux2(priml, primr, s=0.0):
    rhol, ul, pl = priml
    rhor, ur, pr = primr
    csl = cs(priml)
    csr = cs(primr)
...
# Toro eqn 10.75
def hllc_flux(priml, primr, s=0.0):
...
def euler_exact_prim(priml, primr, s=0.0):
...
def euler_exact_flux(priml, primr, s=0.0):
    prim = euler_exact_prim(priml, primr, s=0.0)
    return prim2flux(prim)
