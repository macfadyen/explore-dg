...
if test == 6:
    f = open("FS3.6_3_TCI0_N4000.dat", "r")
    prim_hires = np.genfromtxt(f)
    f.close()
    prim_hires = prim_hires[order : (np.shape(prim_hires)[0] - order), :]
    xan = 0.5 * dx + np.linspace(
        xmin, xmax, num=np.shape(prim_hires)[0], endpoint=False
    )
    # xan = xmin + dx * (np.arange(np.shape(prim_hires)[0]) + 0.5)
    rho_an = prim_hires[:, 0]

if test == 7:
    f = open("FS3.7_3_TCI0_N4000.dat", "r")
    prim_hires = np.genfromtxt(f)
    f.close()
    prim_hires = prim_hires[order : (np.shape(prim_hires)[0] - order), :]
    # xan = xmin + dx * (np.arange(np.shape(prim_hires)[0]) + 0.5)
...
trzn_1D_current = trzn[-1, :]
print(max(trzn_1D_current))
print(np.where(trzn[-1, :] > tci))
print(np.where(trzn[-2, :] > tci))
zone1 = 83
zone2 = 84
zone3 = 85
indicator_field = 0  # density
x1 = zone1 * NUM_FIELDS + indicator_field
x2 = zone2 * NUM_FIELDS + indicator_field
x3 = zone3 * NUM_FIELDS + indicator_field

plt.figure()
# plt.bar(np.arange(order), np.log10(abs(wgts[xx, :])))
plt.loglog(
    np.arange(order),
    abs(wgts[x1, :]),
    "--kD",
